...
function defaultState() {
  ...
  return {
    ...
    retirementScenarios: [],
    retirement: { ...retirementEngine().defaults }
  };
}
...
function normalizePerformanceObservation(observation, index = 0) {
  return performanceSourceEngine().normalizePerformanceObservation(observation, index);
}
...
function validateRetirementInput(input) {
  return retirementEngine().validateRetirementInput(input);
}
...
function performanceSourceEngine() {
  const engine = window.AssetTrailPerformanceSourceEngine;
  if (!engine) throw new Error("성과 자료 검증 엔진을 불러오지 못했습니다.");
  return engine;
}

function allPerformanceEvidence() {
  return performanceSourceEngine().buildEvidence({ performanceObservations: state.performanceObservations,
    events: state.events, baselineDate: state.ledgerMeta?.baselineDate,
    todayKey: localDateInputValue(), benchmarkKey: uiState.performanceBenchmark });
}
...
function aiReviewPerformance(generatedAt = new Date().toISOString()) {
  return performanceSourceEngine().buildReviewPerformance({ performanceObservations: state.performanceObservations,
    events: state.events, baselineDate: state.ledgerMeta?.baselineDate,
    todayKey: localDateInputValue(new Date(generatedAt)) });
}
...
function buildAiReviewInput(generatedAt = new Date().toISOString()) {
  ...
  return aiReviewEngine().buildSnapshotReviewInput({
    ...
    performance: aiReviewPerformance(generatedAt),
    goal: aiReviewGoal(),
...
function retirementEngine() {
  const engine = window.AssetTrailRetirementEngine;
  if (!engine) throw new Error("은퇴 계산 엔진을 불러오지 못했습니다.");
  return engine;
}

function retirementConfigured() {
  return retirementEngine().isConfigured(state.retirement);
}
...
function strongDeterministicFingerprint(prefix, value) {
  return performanceSourceEngine().strongDeterministicFingerprint(prefix, value);
}

function performanceObservationFingerprint(observation) {
  return performanceSourceEngine().performanceObservationFingerprint(observation);
}

function performanceNumbersClose(left, right) {
  return performanceSourceEngine().performanceNumbersClose(left, right);
}

function performanceObservationIdentityValid(observation) {
  return performanceSourceEngine().performanceObservationIdentityValid(observation);
}
...
function performanceEventDate(event) {
  return performanceSourceEngine().performanceEventDate(event);
}
...
function externalPerformanceFlows(startDate, endDate, activeEvents) {
  return performanceSourceEngine().externalPerformanceFlows(startDate, endDate, activeEvents);
}

function performanceInputForRange(bounds) {
  return performanceSourceEngine().buildDataset({
    performanceObservations: state.performanceObservations, events: state.events,
    baselineDate: state.ledgerMeta?.baselineDate, benchmarkKey: uiState.performanceBenchmark, ...bounds
  });
}

function performanceXirr(engine, dataset) {
  return performanceSourceEngine().performanceXirr(engine, dataset);
}
...
function calculateRetirement(input) {
  return retirementEngine().calculateRetirement(input);
}

function presentValueGrowingAnnuity(firstPayment, rate, growth, years) {
  return retirementEngine().presentValueGrowingAnnuity(firstPayment, rate, growth, years);
}

function requiredAnnualReturn(principal, monthlyContribution, target, months) {
  return retirementEngine().requiredAnnualReturn(principal, monthlyContribution, target, months);
}
