...
- 설정은 가장 최근 조회 기록의 `ASSETTRAIL_AI_REVIEW_V2` 종목별 수량·확정 종가·
  환율·원화 평가액, 상대 지표와 고정 프롬프트를 Markdown으로 내보낸다. API·AI 계정
  연동·자동 전송·결과 가져오기·저장은 제공하지 않는다.
...
- 저장 스키마 v8의 신규 조회 기록은 `assettrail.snapshot-valuation.v1`에 자산 ID,
  유효 계좌분류, 종목·수량·적용 종가·가격일·환율·원화 평가액을 함께 보존한다. 과거
  기록에는 현재 가격을 소급 적용하지 않는다.
- v7의 기존 IndexedDB 히스토리 세대는 먼저 읽고 검증한 뒤 v8로 승격해야 하며, 읽기나
...
## 방금 완료한 수정 (2026-09-07 AI 점검 종목별 평가정보)

- AI 점검 계약을 `ASSETTRAIL_AI_REVIEW_V2`로 올리고, 가장 최근 조회 기록에 저장된
  동일 시장·티커·계좌분류의 `quantity`와 확정 종가·가격일·환율·`marketValueKRW`,
  CASH·MANUAL 저장액을 포함한다.
- 패키지는 `snapshotId`·`snapshotCreatedAt`·`valuationStatus`로 근거 기록을 밝히며,
  `asOfDate`는 조회 기록 저장일, `priceAsOf`는 실제 가격일이다. 종목별 근거가 없는
  과거 기록은 현재 가격으로 대체하지 않고 재저장을 안내한다.
- UID·계좌명·원거래·자유 메모 제외와 무네트워크·무저장 계약은 유지한다.
- 월별 실현손익 테스트의 8월 고정 기대를 실행일 기준 연·월로 바꿨다.
- `.venv`를 PATH에 포함한 `npm test`는 Firestore 직전까지 전부 통과했다. Firestore
  Rules 테스트만 로컬 Java 런타임이 없어 미실행이며 PR CI에서 다시 확인해야 한다.
