# 핸드오프 노트 - 월간 자산 점검 제품 이어가기

## 2026-09-08 V3 영숫자 종목코드 수정 검토

- AI export의 KRX 검증이 숫자 6자리만 허용하여 앱이 지원하는 영숫자 6자리 코드를 누락했다. 앱의 기존 계약과 동일한 검증으로 맞췄다.
- 정상 숫자·영숫자·소문자 정규화 및 잘못된 길이·구분자·문자 거부 회귀 검증을 추가했다. 운영 병합·배포는 아직 승인 전이다.
- 상세 검증과 후속 인수 조건은 `docs/sessions/2026-09-08-v3-alphanumeric-tickers.md` 참고.
...
- 현재 IA는 `홈 / 자산 / 기록 / 목표`와 우측 상단 `설정`이다. 포트폴리오·의사결정·
  분석 화면은 `LEGACY`로 숨기되 기존 상태와 Butler·ETF 별도 로컬 데이터는 보존한다.
- 홈의 월간 점검은 같은 달 기록을 갱신한다. 기록 화면은 거래·현금흐름·성과·조회
  이력을 맡고, 목표 화면은 은퇴 시뮬레이터에 집중한다.
- 설정은 가장 최근 조회 기록의 `ASSETTRAIL_AI_REVIEW_V3` 계좌별 종목 수량·확정 종가·
  환율·원화 평가액·저장 당시 계좌명, 상대 지표와 고정 프롬프트를 Markdown으로
  내보낸다. API·AI 계정 연동·자동 전송·결과 가져오기·저장은 제공하지 않는다.
- v7은 조회·성과 이력을 로컬 IndexedDB와 Firestore 월별 chunk로 분리하고 각각
  10,000개까지 보존한다. v6 평면 배열은 백업 후 새 세대로 이전한다. 분리 세대가 없는
  신규·v6의 IndexedDB 미지원 경로와 이동용 JSON은 평면 배열을 유지하지만, 기존 v7
  세대를 읽지 못하면 변경·동기화·전체 내보내기를 차단한다.
- 현재 작업의 테스트·배포 결과는 완료 보고 전까지 확정하지 않는다. 상세 계약은
  `docs/ARCHITECTURE.md`, 검증 명령은 `docs/TESTING.md`를 기준으로 한다.

## 현재 릴리스 게이트 (2026-08-19)

- 강화된 `firestore.rules`는 2026-08-19 12:07:59 KST에 운영 Firebase 프로젝트
  `assettrail-6f676`에 배포했다. 활성 Rules 원문과 작업 트리 파일의 UTF-8 바이트 및
  SHA-256 일치, history chunk 경로와 비활성 세대 삭제 보호를 재확인했다.
- 남은 릴리스 완료 조건은 PR CI의
  정적 런타임 manifest 검증, `main` 병합 후 Pages 배포, 라이브 앱·런타임 파일 확인이다.
  이 조건이 모두 끝나기 전에는 현재 개편을 운영 배포 완료로 기록하지 않는다.

## 이번 릴리스 변경 (2026-09-07 저장 시 최신 확정 종가)

- 조회 기록 저장 직전에 캐시를 우회해 운영 가격표를 다시 받고, 거래소 캘린더·시장
  상태로 독립 확인한 최근 완료 세션의 `FINAL_CLOSE` 인증만 사용한다. 갱신 실패,
  보유 시장 인증 누락·만료, 종목 가격·미국 환율 누락 시 이전 메모리 가격으로 저장하지 않는다.
- 저장 스키마 v9의 신규 조회 기록은 `assettrail.snapshot-valuation.v2`에 자산 ID,
  유효 계좌분류, 사용자가 입력한 계좌명, 종목·수량·적용 종가·가격일·환율·원화 평가액을
  함께 보존한다. 과거 기록에는 현재 가격이나 현재 계좌명을 소급 적용하지 않는다.
- v7·v8의 기존 IndexedDB 히스토리 세대는 먼저 읽고 검증한 뒤 v9로 승격해야 하며, 읽기나
  새 세대 검증이 실패하면 기존 포인터를 유지하고 저장을 차단한다.
- 실제 생성에서 KRX 4,303개, US 24/24개, USD/KRW 1,355.410034와 세 시장의
  2026-09-04 완료 세션을 확인했다. 로컬 전체 테스트는 Firestore Rules 직전까지
  통과했고, Rules는 로컬 Java 부재로 PR CI에서 확인한다.
- 완료 조건은 전체 테스트, 실제 가격 생성, PR CI, main 병합, Pages 배포와 운영
  `prices.json`·저장 동작 확인이다. 이 조건이 끝나기 전에는 운영 배포 완료로 기록하지 않는다.

## 이번 릴리스 변경 (2026-09-07 AI 점검 계좌별 평가정보)

- AI 점검 계약은 `ASSETTRAIL_AI_REVIEW_V3`, 고정 지침은
  `ASSETTRAIL_MONTHLY_REVIEW_PROMPT_V3`를 사용한다. 가장 최근 조회 기록에 저장된
  동일 계좌·시장·티커의 `quantity`와 확정 종가·가격일·환율·`marketValueKRW`,
  CASH·MANUAL 저장액과 저장 당시 `accountName`을 포함한다. `AUTO` 계좌분류는 ISA를
  일반계좌와 분리한다.
- 패키지는 `snapshotId`·`snapshotCreatedAt`·`valuationStatus`로 근거 기록을 밝히며,
  `asOfDate`는 조회 기록 저장일, `priceAsOf`는 실제 가격일이다. 계좌명이 없는 기존
  `assettrail.snapshot-valuation.v1`은 현재 자산과 결합하지 않고
  `MISSING_SNAPSHOT_ACCOUNT_NAMES`로 재저장을 안내한다. 평가 근거 자체가 없는 더 오래된
  기록도 현재 가격으로 대체하지 않는다.
- 계좌명은 계좌별 진단을 위해 포함한다. 계좌명에 계좌번호·이메일 등 민감정보를 적었다면
  사용자가 수정하고 조회 기록을 다시 저장한 뒤 외부 AI에 첨부해야 한다. UID·이메일,
  내부 자산·거래 ID, 원거래와 자유 메모 제외 및 무네트워크·무저장 계약은 유지한다.
- Python 3.13 의존성과 Java 21을 사용한 `npm test` 전체 검증을 통과했다. Firestore
  Rules의 예상된 `PERMISSION_DENIED` 검증도 최종 종료 코드 0으로 통과했다.
- 남은 완료 조건은 PR 전체 CI, main 병합, Pages 배포와 운영 파일의 v9/V3 캐시 키 및
  실제 계좌별 내보내기 확인이다.

## 방금 완료한 수정 (2026-09-07 미국 보유종목 가격 복구)

- 운영 `prices.json`이 2026-08-31 이후 갱신되지 않았고, MU·AVGO·BE·SNDK·ADI·NVDA·
  WDC·INTC·COHR가 리뷰 대상 `tickers.json`에서 빠져 있어 조회 기록 저장이 차단됐다.
- 9개 티커를 정렬된 US 가격 생성 대상에 추가하고 실제 생성에서 US 24/24, 오류 0건,
  KRX 4,303건과 USD/KRW를 확인했다.
- 가격 생성 테스트가 9개 필수 티커의 포함, 정렬, 중복 제거를 회귀 계약으로 확인한다.

## 방금 완료한 수정 (2026-08-19 Butler 가져오기 품질 판정)

- 빈 미래 기간 열은 저장 대상에서 제외하며 누락 셀로 계산하지 않는다.
- Butler 손익계산서의 정상 상세 행은 알 수 없는 지표 경고로 분류하지 않는다.
- 커버리지는 붙여넣은 표에서 지원하는 핵심 지표의 실제 저장 범위를 기준으로 판단한다.
- 미리보기는 오류 개수 대신 최신 실적과 저장 예정 기간·핵심 지표·수치 개수를 보여준다.

## 방금 완료한 수정 (2026-08-14 예수금 잔액 관리)

- **현금 전용 동작** — CASH 자산의 표·카드·상세에서 입금, 출금, 잔액 맞추기와 내역을
  바로 선택할 수 있다. 자산 수정 화면에서는 원장으로 계산된 금액을 계속 잠그되,
  잠금 이유와 올바른 관리 경로를 바로 안내한다.
- **잔액 맞추기** — 사용자가 실제 예수금을 입력하면 `현재 잔액 → 차액 → 반영 후
  잔액`을 먼저 보여준다. 추가납입·출금·배당·이자·수수료·세금은 기존 유형으로,
  최초 등록 오류는 활성 기초잔액의 감사 정정으로 저장한다. 원인 미확인 차액은 사유를
  필수로 남기는 `CASH_ADJUSTMENT`로 저장하고 활성 상태에서는 검증 성과 평가점을
  제한한다.
- **부족금 자동입금 매수** — 선택한 CASH가 부족할 때 부족액을 명시하고, 사용자가
  동의하면 같은 결제일의 `DEPOSIT`과 `BUY`를 한 작업으로 저장한다. 저장 실패 시 둘 다
  롤백하고 되돌리기도 두 이벤트를 함께 취소한다.
- **불변 범위** — 기존 원장 이벤트를 직접 덮어쓰지 않으며 v6 저장 스키마, Firestore
  구조, 표준 CSV v1과 성과 계산 공식은 바꾸지 않았다. 표준 CSV는 원인 미확인 내부
  조정을 새 외부 거래 유형으로 받지 않는다.
- **검증·배포 상태** — 원장·성과·CSV·데이터 내구성·클라우드 테스트가 통과했다.
  Chrome 1440px·1280px·430px·390px에서 잔액 맞추기와 부족금 안내의 가로 넘침,
  중복 ID와 모바일 44px 조작 높이를 확인했다. 작업 브랜치는
  `codex/cash-balance-management`이며 운영 배포와 병합은 하지 않는다. 가격 생성
  Python 테스트는 임시 격리 환경에서 통과했고 Firestore Rules 테스트만 로컬 Java
  런타임 부재로 실행하지 못했다.

## 방금 완료한 수정 (2026-08-07 분석 탭 UX 구조 개선)

- **상태 우선 작업 선택** — 분석 진입 시 기업 실적, ETF 실질노출, AI 보고서의 현재
  상태를 3개 요약 카드에서 먼저 확인한다. 저장된 스냅샷·펀드 수, 계산 가능 여부,
  익명 근거 수와 품질을 기존 로컬 데이터에서 읽어 표시하며 계산식과 저장 구조는
  변경하지 않았다.
- **한 번에 한 작업** — 기존에 서로 다른 높이로 병렬 배치되던 Butler와 ETF, 별도
  하단의 AI 보고서를 하나의 선택 상세 영역으로 통합했다. 작업 선택을 바꾸면 해당
  패널만 표시하며 기업 실적 → ETF → AI 순서를 유지한다. 화살표·Home·End와
  Enter·Space 키 선택, 선택 패널 제목 포커스와 상태 안내를 제공한다.
- **주 행동과 데이터 관리 분리** — Butler 열기, ETF 구성 파일 가져오기, ChatGPT
  전달문 복사는 각 패널 헤더의 주 행동으로 유지했다. 별도 로컬 백업·복원·비우기,
  표준 양식, 근거 재계산·JSON은 `데이터 관리` 펼침 안으로 옮겨 기본 화면 밀도를
  낮췄다. 기존 요소 ID와 이벤트·데이터 계약은 유지했다.
- **보고서 점진 공개** — 결정론/검증 AI 보고서의 문장과 근거 연결 수는 바로 읽되,
  FACT ID와 evidence ID는 각 항목의 `기술 근거 보기`에서만 펼쳐 보이도록 바꿨다.
- **모바일 안정화** — 720px 이하에서는 3개 작업 선택을 상단 스티키 전환부로 압축하고
  선택 상세만 표시한다. 표는 기존 가로 스크롤을 유지하고 패널 CTA와 데이터 관리
  버튼은 전체 폭으로 쌓는다. 1440px·1280px·430px·390px 실화면에서 가로 넘침 없음,
  작업 선택·포커스 이동, 축소 상태를 확인했고 브라우저 오류는 없었다.
- **검증·배포 상태** — `npm run check:js`, `npm run test:stage5`,
  `npm run test:prices`가 통과했다. 변경은 `codex/analysis-layout-stage2` 브랜치에 있으며
  커밋·푸시·PR·머지는 아직 하지 않았다.
