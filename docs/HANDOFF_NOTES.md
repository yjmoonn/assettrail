...
- 설정은 `ASSETTRAIL_AI_REVIEW_V2` 종목별 수량·원화 평가액, 상대 지표와 고정
  프롬프트를 Markdown으로 내보낸다. API·AI 계정 연동·자동 전송·결과 가져오기·저장은
  제공하지 않는다.
...
- AI 점검 계약을 `ASSETTRAIL_AI_REVIEW_V2`로 올리고, 동일 시장·티커의 계좌별 보유를
  합산한 `quantity`와 최신 가격·환율 기준 `marketValueKRW`를 포지션마다 포함한다.
- 패키지 `asOfDate`는 포함 가격일 중 가장 최근 날짜, 각 포지션의 실제 가격일은
  `priceAsOf`로 명시한다. 기존 과거 조회기록은 종목별 내역이 없어 소급 복원하지 않는다.
