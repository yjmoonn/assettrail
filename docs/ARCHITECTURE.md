...
| `services/analysis-api/` | AI 판단, 사용자별 사용량, 분석 이력과 비공개 PDF를 담당하는 Cloud Run 서비스 |
...
| AI 한도 | `users/{uid}/analysisEntitlements/primary` | 관리자/분석 API 서버 |
| AI 월 사용량 | `users/{uid}/analysisUsage/{YYYY-MM}` | 분석 API 서버 |
...
5. 사용자가 별도 동의 후 구조 분석 또는 최신 시장 맥락 모드로 AI 보고서를 요청한다.
6. 서버가 월 한도를 예약하고 계좌명을 익명화한 최소 계산 결과만 OpenAI Responses API에 전달한다.
7. 구조화된 AI 판단과 Noto CJK/Chromium 기반 기관형 5+2페이지 PDF를 생성한다.
8. 판단 JSON은 분석 이력에, PDF는 사용자별 비공개 Firebase Storage 경로에 저장한다.

분석 API 주소는 `firebase-config.js`의 `analysisApiBaseUrl`에 둔다. 비어 있으면 수치 분석과 로컬 이력은 작동하지만 서버 이력과 AI 보고서 생성은 비활성 상태다. AI 키는 브라우저나 저장소가 아닌 Google Secret Manager에서만 Cloud Run에 주입한다.
