...
| `historyMeta` | v7부터 사용하는 활성 히스토리 세대, 개수, chunk 수와 내용 fingerprint |
...
prefix로 안정적으로 shard한다. 평가내역은 스냅샷당 최대 500개이며 단일 스냅샷 자체가
  220KiB 안전 한도를 넘으면 잘라 저장하지 않고 저장 전에 실패시킨다. 각 chunk digest와 전체
content fingerprint를 검증한다.
...
`FEE`, `TAX`, `CASH_ADJUSTMENT`, `SPLIT`, `VALUATION`, `FX`와 내부
`OPENING_BALANCE`, `CANCEL` 이벤트를
...
예수금 잔액 맞추기는 차액 원인이 확인되면 기존 현금흐름 유형을 사용하고, 최초 등록
오류는 활성 `OPENING_BALANCE`를 감사 정정한다. 원인 미확인 차액만 원화 전용
`CASH_ADJUSTMENT`로 기록하며, 활성 조정은 잔액 투영에는 포함하되 검증 성과 평가점을
`INCOMPLETE`로 제한한다. 부족금 자동입금 매수는 같은 결제일의 `DEPOSIT`과 `BUY`를
순서가 보장된 한 작업으로 추가하고 되돌릴 때 두 이벤트에 `CANCEL`을 함께 남긴다.
