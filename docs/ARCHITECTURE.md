...
저장 데이터는 현재 `schemaVersion: 5`를 사용한다. 버전이 없거나
`schemaVersion: 1~4`인 기존 데이터는 검증 가능한 원본 백업을 먼저 만든 뒤 v5로
마이그레이션한다. 기존 자산마다 원장 기준일의 `OPENING_BALANCE`를 만들며 매수 시점과
과거 환율을 추정하지 않는다. 로컬 마이그레이션 백업은 별도 키에 쓴 뒤 다시 읽어
...
구버전 앱이 v5 데이터를 읽고 새 필드나 원장 하위 컬렉션을 제거한 채 다시 쓰지 못하도록
...
변경 불가능한 백업으로 남긴 뒤 진행한다. 스키마 v5와 원장 Rules를 모르는 이전 앱은
v5 주 문서를 감지하면 읽기·쓰기를 중단한다.
