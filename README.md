...
7. 추가 해석이 필요하면 설정에서 `AI 점검 패키지 내보내기`를 누른다. 앱은 고정
   프롬프트, 가장 최근에 저장한 조회 기록 기준 계좌별 종목 수량·확정 종가·환율·원화
   평가액과 사용자가 입력한 계좌명, 상대 지표를 담은
   `ASSETTRAIL_AI_REVIEW_V3` Markdown 파일만 만들며,
   사용자가 원하는 AI 서비스에 직접 첨부한다. 앱이 AI API를 호출하거나 결과를
   다시 가져와 저장하지 않는다.
   - 계좌명에 계좌번호·이메일 등 민감정보를 적었다면 먼저 계좌명을 수정하고 조회
     기록을 다시 저장한 뒤 외부 AI에 첨부한다.
...
가격표는 GitHub Actions가 미국 조기·정규장과 Yahoo 원화 환율 마감의 서머타임·
표준시 전환을 포괄하도록 02:20·03:20·05:20·06:20·08:20·09:20 KST에 확인하고,
국내 정규장과 지연 개장 특수일을 포괄하도록 평일 15:45·16:40 KST에 확인한다.
...
- 생성기는 거래소 캘린더와 시장 상태로 최근 완료 세션을 먼저 확정하고, 장중 일봉을
  제외하거나 직전 완료 세션 종가로 되돌린 뒤 각 가격·환율을 `FINAL_CLOSE`로 인증한다.
  앱은 조회 기록 저장 직전에 가격표를 다시 내려받고, 보유 시장의 확정 종가 인증이
  없거나 해당 시장의 다음 마감으로 인증이 만료됐으면 저장을 중단한다.
...
- 설정의 AI 점검 내보내기는 `ASSETTRAIL_AI_REVIEW_V3` 데이터와 변경할 수 없는
  `ASSETTRAIL_MONTHLY_REVIEW_PROMPT_V3` 지침을 하나의 Markdown 파일로 만든다.
- 포함 범위는 가장 최근 조회 기록에 보존된 종목별 수량·확정 종가·가격일·환율·
  원화 평가액·계좌분류·저장 당시 사용자가 입력한 계좌명, CASH·MANUAL 저장액,
  자산군·포지션 상대 비중, 집중도, 검증된 성과율, 은퇴 목표 비율과 점검 상태다.
  UID·이메일, 내부 자산·거래 ID, 원거래와 자유 메모는 제외한다. 다만 계좌명 필드는
  그대로 포함되므로 계좌번호·이메일 등 민감정보를 계좌명에 적었다면 수정 후 조회
  기록을 다시 저장해야 한다.
- 계좌명이 없는 `assettrail.snapshot-valuation.v1` 기록은 현재 자산과 결합해 이름을
  추정하지 않고 `MISSING_SNAPSHOT_ACCOUNT_NAMES`로 표시한다. 계좌별 진단에는 새 조회
  기록 저장이 필요하다. 종목별 평가 근거가 없는 더 오래된 기록도 현재 가격으로
  대체하지 않는다.
...
히스토리와 은퇴 설정을 로그인 사용자별로 동기화한다. 현재 저장 스키마는 v9이며 v8
...
v9에서 새로 저장하는 조회 기록은 `assettrail.snapshot-valuation.v2`에 총액과 함께
양수 수량의 시장 자산과 CASH·MANUAL 자산의 저장 당시 수량·확정 종가·환율·원화
평가액·유효 계좌분류·사용자가 입력한 계좌명을 보존한다. `AUTO` 계좌분류는 ISA를
별도로 식별한다. 시장
자산이 없는 기록도 수동 평가내역을 별도 방법론으로 남기며, 기존 기록에는 현재 가격을
소급 적용하지 않는다.
조회 스냅샷과 성과 평가점은 최대 각각 10,000개까지 저장하고 가져온다. 로컬에서는
...
사용한다. 기존 v7·v8 주 상태가 가리키는 IndexedDB 세대를 읽지 못하면 불완전 백업을 만들지
...
- 가격표는 하루 두 번의 마감 후 스케줄, 수동 가격 업데이트, 가격 입력/생성기 파일 변경 시에만 새로 생성한다.
