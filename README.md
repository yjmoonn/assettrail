...
7. 추가 해석이 필요하면 설정에서 `AI 점검 패키지 내보내기`를 누른다. 앱은 고정
   프롬프트, 가장 최근에 저장한 조회 기록 기준 종목별 수량·확정 종가·환율·원화
   평가액과 상대 지표를 담은
   `ASSETTRAIL_AI_REVIEW_V2` Markdown 파일만 만들며,
...
- 설정의 AI 점검 내보내기는 `ASSETTRAIL_AI_REVIEW_V2` 데이터와 변경할 수 없는
  `ASSETTRAIL_MONTHLY_REVIEW_PROMPT_V2` 지침을 하나의 Markdown 파일로 만든다.
- 포함 범위는 가장 최근 조회 기록에 보존된 종목별 수량·확정 종가·가격일·환율·
  원화 평가액·계좌분류, CASH·MANUAL 저장액, 자산군·포지션 상대 비중, 집중도,
  검증된 성과율, 은퇴 목표 비율과 점검 상태다. 계좌명, 사용자 식별자, 거래 행과
  자유 메모는 제외한다. 종목별 근거가 없는 과거 기록은 현재 가격으로 대체하지 않는다.
...
히스토리와 은퇴 설정을 로그인 사용자별로 동기화한다. 현재 저장 스키마는 v8이며 v7
...
v8에서 새로 저장하는 조회 기록은 총액과 함께 양수 수량의 시장 자산과 CASH·MANUAL
자산의 저장 당시 수량·확정 종가·환율·원화 평가액·유효 계좌분류를 보존한다. 시장
...
사용한다. 기존 v8 주 상태가 가리키는 IndexedDB 세대를 읽지 못하면 불완전 백업을 만들지
